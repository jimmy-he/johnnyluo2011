	...
	public String sfYanZheng() {
		...
		String groupinput=
			...
			"<ACLife>"+
				...
				"</ACLifeRequest>"+
			"</ACLife>";
		String url ="http://192.168.1.33:8080/GIP/services/GIPService?wsdl";
		String tradecode ="";
		String tradetype ="04";
			...
			if(cBean.getOnenumber()!=null||cBean.getGroupidnumber()!=null||cBean.getCardgroupidnumber()!=null){
				...
				try {
					...
					if(null != one_policy_no){
						...
						logger.info("此保单号存在，验证通过！并且缓存----此保单号属于个险的保单号"+ID_NO);
						System.out.println("缓存的6位保单号码："+ID_NO);
						logger.info("缓存的6位保单号码："+ID_NO);
						cBean.setSix_one_no(ID_NO);
						String xstrString=null;
						cBean.setSix_group_no(xstrString);//此保单号属于团单，清理掉团单号
						cBean.setSix_card_no(xstrString);//此保单号属于团单，清理掉卡单号
				...
				//------------如果保单号在团险中存在------------团单+卡单-----
			if(cBean.getGroupidnumber()!=null||cBean.getCardgroupidnumber()!=null){
				/*
				 * 拿取团险中存在的保单号
				 */						
				System.out.println("-------------进入团险保单验证---团单**卡单----------");
				logger.info("-------------进入团险保单验证---团单**卡单----------");
				SAXReader saxReader = new SAXReader(false);
				...
				try {
					...
					Node card_policy_no = root.selectSingleNode("ACLifeResponse/RetContent/Response/ContCardInfo/ContCard[ends-with(ContCardNo, '" + ID_NO + "')]");
					if(null != group_policy_no||null !=card_policy_no){
						out.append("Y|");	
						if(null != group_policy_no){
						System.out.println("此保单号存在，验证通过！并且缓存----此保单号属于团险的保单号"+ID_NO);
						logger.info("此保单号存在，验证通过！并且缓存----此保单号属于团险的保单号"+ID_NO);
						System.out.println("缓存的6位保单号码："+ID_NO);
						logger.info("缓存的6位保单号码："+ID_NO);
						String xstrString=null;
						cBean.setSix_group_no(ID_NO);
						cBean.setSix_one_no(xstrString);  //此保单号属于团单，清理掉个险保单号
						cBean.setSix_card_no(xstrString);  //此保单号属于团单，清理掉卡单号
//						out.append("Y|");
//						logger.info("Y|");
						}
					if(null !=card_policy_no){
							System.out.println("此保单号存在，验证通过！并且缓存----此保单号属于卡单的保单号"+ID_NO);
							logger.info("此保单号存在，验证通过！并且缓存----此保单号属于卡单的保单号"+ID_NO);
							System.out.println("缓存的6位保单号码："+ID_NO);
							logger.info("缓存的6位保单号码："+ID_NO);
							cBean.setSix_card_no(ID_NO);
							String xstrString=null;
							//cacheMgr.remove("ChannelNo:" + channelNo,cBean.getSix_group_no());
							cBean.setSix_one_no(xstrString);//此保单号属于团单，清理掉个险保单号
							cBean.setSix_group_no(xstrString);//此保单号属于团单，清理掉团单号
//							out.append("Y|");
						}
						...
						out.append("N|输入错误，请重新输入，返回上层菜单请按星号，结束请挂机！");
						logger.info("N|输入错误，请重新输入，返回上层菜单请按星号，结束请挂机！");
					}
				} catch (DocumentException e) {
					e.printStackTrace();
				}		
...
//			}
	...
	public String bdXinXi() {
		...
		logger.info("6位个险保单号码："+one_no);
		String grp_no=cBean.getSix_group_no();
		System.out.println("6位团险团单号码："+grp_no);
		logger.info("6位团险团单号码："+grp_no);
		String card_no=cBean.getSix_card_no();
		System.out.println("6位团险卡单号码："+card_no);
		logger.info("6位团险卡单号码："+card_no);
		if(one_no!=null&&one_no!=""  ||   grp_no!=null&&grp_no!=""  ||   card_no!=null&&card_no!=""){
				if(one_no!=null&&one_no!="")
				{	
					System.out.println("---------个险有此基本信息------------");
					logger.info("---------个险有此基本信息------------");
					/*
					 * 拿取个险中的数据
					 */
					
					String company_code=cBean.getCompany_code();
					System.out.println("拿取个险中的分公司号码:"+company_code);
					logger.info("拿取个险中的分公司号码:"+company_code);
					String policy_no=cBean.getPolicy_no();
					System.out.println("拿取个险中的完整保单号码："+policy_no);
					logger.info("拿取个险中的完整保单号码："+policy_no);
						...
						try {
							...
							if(rs.next()){
								...
								logger.info("Y|保单"+bdh+zhuangtai+", 保费"+jiaofeileixingString+amount+"元"+", 下次应缴日为"+yearString+"年"+mothString+"月"+dateString+"日"+"   重听   请按零,  返回上层菜单请按星号键,  如需查询其他保单请按  一,  结束请挂机");
									...
									logger.info("Y|保单"+bdh+zhuangtai+", 保费"+jiaofeileixingString+amount+"元"+",   重听   请按零,  返回上层菜单请按星号键,  如需查询其他保单请按  一,  结束请挂机");
								}else if(LogicNumbers.equals("3")){
									System.out.println("Y|保单"+bdh+zhuangtai+"   保费是"+amount+"元");
									out.append("Y|保单"+bdh+zhuangtai+","+","+"   重听   请按零,  返回上层菜单请按星号键,  如需查询其他保单请按  一,  结束请挂机");
									logger.info("Y|保单"+bdh+zhuangtai+","+"   重听   请按零,  返回上层菜单请按星号键,  如需查询其他保单请按  一,  结束请挂机");
								}
							}else {
								//out.append("存储过程中没有拿到值！");
								out.append("Y|此保单号没有保单，返回上层菜单请按星号键,  如需查询其他保单请按  一,  结束请挂机！");
								System.out.print("存储过程中没有拿到值！");
								logger.info("存储过程中没有拿到值！");
							}					
						}catch (SQLException e) {
							e.printStackTrace();
							System.out.println("存储过程调用失败！");
							logger.debug("存储过程调用失败！");
						}
				...
				if(grp_no!=null&&grp_no!="")
				{
					...
					logger.info("团险有此基本信息");
					SAXReader saxReader = new SAXReader(false);
					String groupxml=cBean.getGroupxml();
					StringBuffer strbuf = new StringBuffer();
					Document doc;
					Node group_policy_no = null;
					String riskCvalidate=null;
				                    ...
				                    for (Node insuredNode : insuredNodes) {
				                        ...
				                        if (null != grpRiskInfoNodes) {
				                            ...
				                            for (Node grpRiskInfoNode : grpRiskInfoNodes) {
				                                ...
				                                riskCvalidate = grpRiskInfoNode.valueOf("RiskCvalidate");
				                                riskEnddate = grpRiskInfoNode.valueOf("RiskEnddate");
			...
			logger.info(strbuf.toString()+"销售单位"+","+ riskSaleCompany+"重听   请按零,  返回上层菜单请按星号键,  如需查询其他保单请按  一,  结束请挂机");
					}catch (Exception e)
						{
					...
					try {
							...
							card_policy_no = root.selectSingleNode("ACLifeResponse/RetContent/Response/ContCardInfo/ContCard[ends-with(ContCardNo, '" + card_no + "')]");							
				                ...
				                if (null != insuredNodes&& !insuredNodes.isEmpty()) {
				                    System.out.println("==> Insured size: " + insuredNodes.size());                  
				                    for (Node insuredNode : insuredNodes) {                   	
				                        strbuf.append(insuredNode.valueOf("ContCardState") + "状态"+",");				                        
				                        strbuf.append("保险费用"+card_policy_no.valueOf("ContCardPrem")+"元"+",");				                      
				                        // 获取GrpRiskInfo
				                ...
				                out.append(strbuf.toString()+","+"保险责任生效日期" + riskCvalidate + ","+"保险责任终止日期" + riskEnddate + ","+"销售单位："+card_policy_no.valueOf("RiskSaleCompany")+","+"重听   请按零,  返回上层菜单请按星号键,  如需查询其他保单请按  一,  结束请挂机");
				                logger.info(strbuf.toString()+","+"销售单位："+card_policy_no.valueOf("RiskSaleCompany")+"重听   请按零,  返回上层菜单请按星号键,  如需查询其他保单请按  一,  结束请挂机");
					}catch (Exception e)
						{
						System.out.println("卡单的保单信息出错啦！");
						}
				}	
				
			}else{
				System.out.println("此保单号不存在保单信息！-------00000");
				logger.info("此保单号不存在保单信息！-------00000");
				out.append("N|输入错误，请重新输入，返回上层菜单请按星号，结束请挂机！");
			}
	...
	public String liPei() {
		...
		logger.info("进入理赔信息查询------3333");
		System.out.println("操作时间！--"+new Date());
		CacheMgr cacheMgr = CacheMgr.getInstance();
		String channelNo = super.getChannelNo();
		System.out.println("在理赔信息查询中拿到channelNo:"+channelNo);
		logger.info("在理赔信息查询中拿到channelNo:"+channelNo);
		AbstractCacheBean cBean = (AbstractCacheBean) cacheMgr.get("ChannelNo:"+ channelNo);
		...
		logger.info("完整个险保单号码："+policy_no);
		...
		{	
			...
			try {
				...
				if(onenumber!=null){
					...
					logger.info("保单号码为："+baodanString);
	...
	logger.info(xString);
										 ...
										 if(!jiean_dte.equals("2999-12-31")){
...
logger.info(xString);
				...
				logger.info("(LIFE_CLMD.equals(01) && COVERAGE.equals(01) && RIDER.equals(00))====此条件不成立！");
						logger.info("死亡理赔中没有符合条件的理赔信息！");
						}
					}else {
						//System.out.println("死亡理赔半年内没有符合条件的理赔信息！");
						logger.info("死亡理赔半年内没有符合条件的理赔信息！");
							} 			
						 }
					 }							 							 
					}	
						...
						logger.info("list的长度为："+list.size()+"没有满足条件的保单！"); 
						out.append("N|对不起,您要查询的半年内的理赔保单不存在，如须查询其它保单请按1。返回上层菜单请按星号键，结束请挂机");
						logger.info("N|对不起,您要查询的半年内的理赔保单不存在，如须查询其它保单请按1。返回上层菜单请按星号键，结束请挂机");
						}else{
							...
							out.append("重听请按0,返回上层菜单请按*号，如须查询其它保单请按1，结束请挂机");
							logger.info("重听请按0,返回上层菜单请按*号，如须查询其它保单请按1，结束请挂机");
						}	                    
	                    
				}else{
					System.out.println("此保单号码没有保单信息！");
					out.append("N|此保单号码没有保单信息,返回上层菜单请按 星号,结束请挂机！");
					logger.info("N|此保单号码没有保单信息,返回上层菜单请按 星号,结束请挂机！");
				}												
				}else{
					System.out.println("此保单号码没有保单信息！");
				}
				...
				logger.info("个险接口调用失败");
				out.append("N|个险接口调用失败");
				}
		}else{
			out.append("N|对不起,您要查询的理赔保单不存在，如须查询其它保单请按1。返回上层菜单请按星号键，结束请挂机");
			logger.info("N|对不起,您要查询的理赔保单不存在，如须查询其它保单请按1。返回上层菜单请按星号键，结束请挂机");
		}
	...
	public  String ZHjianChe() {
		...
		logger.info("拿到传入的工号和渠道号串，并进行拆解---"+xString);
		...
		logger.info("获取输入的渠道类型代号：" + qudao);
		//System.out.println("获取代理人工号：" + agentno);	
		logger.info("获取代理人工号：" + agentno);	
		Connection conn = DBConnection.getConnection();
		ResultSet rs = null;
		CallableStatement statement = null;
		try {
			...
			logger.info("您输入的条件超出范围！请查证后重新输入！");
		}try {
			rs.close();
			statement.close();
			conn.close();
	...
	public String PolicyStatus() {
		...
		logger.info("修改后的保单号为："+afterbaodanString);
		java.util.Date nowdate = new java.util.Date();
		java.text.SimpleDateFormat hmfromat = new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		String strNowTime = hmfromat.format(nowdate);
		String TradeDate = strNowTime.substring(0, 10);
		...
		try {           
			...
			logger.info("得到保单号码+分公司号调用webservice的数据---" + ret);
			//out.append(ret);
		} catch (Exception e) {
			System.err.println(e.toString());
		...
		logger.info("---------保单号码调用webservice成功----------");
		SAXReader saxReader = new SAXReader(false);
		Document onedoc=null;
		Element onenumber = null;
		Element clm_regp_t = null;
...
logger.info(xString);
					...
					out.append("N|此保单号没有理赔信息,返回上层菜单请按 星号,结束请挂机!");
					logger.info("N|此保单号没有理赔信息,返回上层菜单请按 星号,结束请挂机!");
					}	
			}else{
	 ...
	 * 查询保单状态、保费、下次应缴日期
	 * <p>1.0.0.0 罗尧创建 2010-10-26</p>
	 * @see com.crm.avivacofco.In.CacheBean.AbstractCacheBean#bdzlxqsf()
	 */
	@Override
	public  String bdzlxqsf(){		
		//System.out.println("保单资料及续期收费查询方法------88888888");
		logger.info("保单资料及续期收费查询方法------88888888");
		System.out.println("操作时间！--"+new Date());
		StringBuffer out=new StringBuffer();
		CacheMgr cacheMgr = CacheMgr.getInstance();
		//System.out.println("-----------"+cacheMgr);	
		...
		try {
			...
			if(rs.next()){
				...
				logger.info("第一个存储过程缓存的代理人工号:"+onedlr);
					//进行判断
...
/*----------------------------------------新增六个接口，进行数据库操作，查询各险种利率----------------------------------------------*/
	/* 万能险当月结算利率查询
	 * commandNo--111111
	 * 查询万能险当月结算利率,合成语音
	 * <p>1.0.0.0 罗尧创建 2010-12-26</p>
	 * @see com.crm.avivacofco.In.CacheBean.AbstractCacheBean#AllPowerfulType()
	 */
	@Override
	public String AllPowerfulType() {
		//System.out.println("红利储存利率查询方法------111111");
		logger.info("-----万能险当月结算利率查询------111111");
		System.out.println("操作时间！--"+new Date());
		StringBuffer out=new StringBuffer();
		CacheMgr cacheMgr = CacheMgr.getInstance();
		String channelNo = super.getChannelNo();
		...
		try {
			...
			String sql="SELECT UNIT_VIRTUAL_FUND,START_DTE,RATE_01 FROM ODSCIF.CD_IDX_VALUE_T where type_id = 230300 and END_DTE='2999-12-31'AND COMPANY_CODE='1' and unit_virtual_fund = 'UFNF'";
			...
			while(rs.next()){
				...
				lilv=rs.getObject(3).toString().substring(0, 4);  //利率
		String outString="Y|"+"中英人寿保险有限公司于"+","+date+","+"公布中英人寿万能型保险的"+","+"上月结算利率为百分之"+lilv+",本次公告的数值，仅为上月的投资利率，并不代表未来的投资收益";
		logger.info(outString);
		out.append(outString+"，重听   请按零,  返回上层菜单请按星号键, 结束请挂机");
			}
	...
	/* 金彩连连投资连结保险，金芒果投资连接保险
	 * commandNo--222221
	 * 查询金彩连连投资连结保险，金芒果投资连接保险利率,合成语音
	 * <p>1.0.0.0 罗尧创建 2010-12-26</p>
	 * @see com.crm.avivacofco.In.CacheBean.AbstractCacheBean#WonderfulRepeatedly()
	 */
	@Override
	public String WonderfulRepeatedly() {
		//System.out.println("金彩连连投资连结保险，金芒果投资连接保险------222222");
		logger.info("-----金彩连连投资连结保险，金芒果投资连接保险------222222");
		System.out.println("操作时间！--"+new Date());
		StringBuffer out=new StringBuffer();
		CacheMgr cacheMgr = CacheMgr.getInstance();
		String channelNo = super.getChannelNo();
		...
		try {
			Object date=null;
			Object mairujia=null;
			Object maichujia=null;
			Object mairujia2=null;
			Object maichujia2=null;
			Object mairujia3=null;
			Object maichujia3=null;
			Object mairujia4=null;
			Object maichujia4=null;
			Object mairujia5=null;
			Object maichujia5=null;
			Class.forName("com.ibm.db2.jcc.DB2Driver");
			con = DriverManager.getConnection("jdbc:db2://10.145.6.80:50000/odsdev","db2call","db2call");
			String sql="select  TYPE_ID,UNIT_OFFER_PRICE,UNIT_BID_PRICE,UNIT_VIRTUAL_FUND,(select CODE_Desc from ODSCIF.CD_CODE_T B where A.COMPANY_CODE =B.COMPANY_CODE and A.UNIT_VIRTUAL_FUND =B.DESC_ITEM and B.code_scheme ='T5515' ),EFF_DTE, UPDTE_DTE from ODSCIF.CD_IDX_VALUE_T A where company_code='1' and UNIT_VIRTUAL_FUND = 'FUN1' and VALID_FLG ='1'and eff_dte between (current date-  10  DAYS) and current date order by eff_dte desc fetch first rows only  with ur";
			String sql2="select  TYPE_ID,UNIT_OFFER_PRICE,UNIT_BID_PRICE,UNIT_VIRTUAL_FUND,(select CODE_Desc from ODSCIF.CD_CODE_T B where A.COMPANY_CODE =B.COMPANY_CODE and A.UNIT_VIRTUAL_FUND =B.DESC_ITEM and B.code_scheme ='T5515' ),EFF_DTE, UPDTE_DTE from ODSCIF.CD_IDX_VALUE_T A where company_code='1' and UNIT_VIRTUAL_FUND = 'FUN2' and VALID_FLG ='1'and eff_dte between (current date-  10  DAYS) and current date order by eff_dte desc fetch first rows only  with ur";
			String sql3="select  TYPE_ID,UNIT_OFFER_PRICE,UNIT_BID_PRICE,UNIT_VIRTUAL_FUND,(select CODE_Desc from ODSCIF.CD_CODE_T B where A.COMPANY_CODE =B.COMPANY_CODE and A.UNIT_VIRTUAL_FUND =B.DESC_ITEM and B.code_scheme ='T5515' ),EFF_DTE, UPDTE_DTE from ODSCIF.CD_IDX_VALUE_T A where company_code='1' and UNIT_VIRTUAL_FUND = 'FUN3' and VALID_FLG ='1'and eff_dte between (current date-  10  DAYS) and current date order by eff_dte desc fetch first rows only  with ur";
			String sql4="select  TYPE_ID,UNIT_OFFER_PRICE,UNIT_BID_PRICE,UNIT_VIRTUAL_FUND,(select CODE_Desc from ODSCIF.CD_CODE_T B where A.COMPANY_CODE =B.COMPANY_CODE and A.UNIT_VIRTUAL_FUND =B.DESC_ITEM and B.code_scheme ='T5515' ),EFF_DTE, UPDTE_DTE from ODSCIF.CD_IDX_VALUE_T A where company_code='1' and UNIT_VIRTUAL_FUND = 'FUN4' and VALID_FLG ='1'and eff_dte between (current date-  10  DAYS) and current date order by eff_dte desc fetch first rows only  with ur";
			String sql5="select  TYPE_ID,UNIT_OFFER_PRICE,UNIT_BID_PRICE,UNIT_VIRTUAL_FUND,(select CODE_Desc from ODSCIF.CD_CODE_T B where A.COMPANY_CODE =B.COMPANY_CODE and A.UNIT_VIRTUAL_FUND =B.DESC_ITEM and B.code_scheme ='T5515' ),EFF_DTE, UPDTE_DTE from ODSCIF.CD_IDX_VALUE_T A where company_code='1' and UNIT_VIRTUAL_FUND = 'FUNE' and VALID_FLG ='1'and eff_dte between (current date-  10  DAYS) and current date order by eff_dte desc fetch first rows only  with ur";
			stmt=con.createStatement();
			
			rs=stmt.executeQuery(sql);
			while(rs.next()){//积极型账户
				date=rs.getObject(6);
				mairujia= rs.getObject(2).toString();
				maichujia= rs.getObject(3).toString();
				//System.out.println(date);
			}
			rs=stmt.executeQuery(sql2);
			while(rs.next()){//指数型账户
				mairujia2=rs.getObject(2).toString();
				maichujia2= rs.getObject(3).toString();   
				
			}
			rs=stmt.executeQuery(sql3);
			while(rs.next()){//平衡型账户
				mairujia3= rs.getObject(2).toString();
				maichujia3= rs.getObject(3).toString();  
			}
			rs=stmt.executeQuery(sql4);
			while(rs.next()){//稳健型账户
				mairujia4= rs.getObject(2).toString();
				maichujia4= rs.getObject(3).toString();   
				System.out.println(mairujia4);
			}
			rs=stmt.executeQuery(sql5);
			while(rs.next()){//避险型投资帐户
				mairujia5= rs.getObject(2).toString();
				maichujia5=rs.getObject(3).toString();   
				System.out.println(mairujia5);
			}
			
			String outString="Y|"+"中英人寿《金彩连连投资连结保险》、《金芒果投资连结保险》" +
					"价格公告如下：计价日"+","+date+","+
					"积极型账户买入价"+","+mairujia+",卖出价"+","+maichujia+","+
					"指数型账户买入价"+","+mairujia2+",卖出价"+","+maichujia2+","+
					"平衡型账户买入价"+","+mairujia3+",卖出价"+","+maichujia3+","+
					"稳健型帐户买入价"+","+mairujia4+",卖出价"+","+maichujia4+","+
					"避险型投资帐户买入价"+","+mairujia5+",卖出价"+","+maichujia5+","+
					"本次投资单位价格公告，仅反应各投资帐户以往的投资业绩，并不代表未来的投资收益。";
			logger.info(outString);
			out.append(outString+"重听   请按零,  返回上层菜单请按星号键, 结束请挂机");
		} catch (ClassNotFoundException e) {
			// TODO Auto-generated catch block
	...
	/* 金彩连连投资连结保险B，金芒果投资连接保险B
	 * commandNo--222222
	 * 查询金彩连连投资连结保险B，金芒果投资连接保险B利率,合成语音
	 * <p>1.0.0.0 罗尧创建 2010-12-26</p>
	 * @see com.crm.avivacofco.In.CacheBean.AbstractCacheBean#WonderfulRepeatedlyB()
	 */
	@Override
	public String WonderfulRepeatedlyB() {
		//System.out.println("金彩连连投资连结保险B，金芒果投资连接保险B------22222B");
		logger.info("-----金彩连连投资连结保险B，金芒果投资连接保险B------22222B");
		System.out.println("操作时间！--"+new Date());
		StringBuffer out=new StringBuffer();
		CacheMgr cacheMgr = CacheMgr.getInstance();
		String channelNo = super.getChannelNo();
		...
		try {
			String date=null;
			String mairujia=null;
			String maichujia=null;
			String mairujia2=null;
			String maichujia2=null;
			String mairujia3=null;
			String maichujia3=null;
			String mairujia4=null;
			String maichujia4=null;
			Class.forName("com.ibm.db2.jcc.DB2Driver");
			con = DriverManager.getConnection("jdbc:db2://10.145.6.80:50000/odsdev","db2call","db2call");
			String sql="select  TYPE_ID,UNIT_OFFER_PRICE,UNIT_BID_PRICE,UNIT_VIRTUAL_FUND,(select CODE_Desc from ODSCIF.CD_CODE_T B where A.COMPANY_CODE =B.COMPANY_CODE and A.UNIT_VIRTUAL_FUND =B.DESC_ITEM and B.code_scheme ='T5515' ),EFF_DTE, UPDTE_DTE from ODSCIF.CD_IDX_VALUE_T A where company_code='1' and UNIT_VIRTUAL_FUND = 'FUNA' and VALID_FLG ='1'and eff_dte between (current date-  10  DAYS) and current date order by eff_dte desc fetch first rows only  with ur";
			String sql2="select  TYPE_ID,UNIT_OFFER_PRICE,UNIT_BID_PRICE,UNIT_VIRTUAL_FUND,(select CODE_Desc from ODSCIF.CD_CODE_T B where A.COMPANY_CODE =B.COMPANY_CODE and A.UNIT_VIRTUAL_FUND =B.DESC_ITEM and B.code_scheme ='T5515' ),EFF_DTE, UPDTE_DTE from ODSCIF.CD_IDX_VALUE_T A where company_code='1' and UNIT_VIRTUAL_FUND = 'FUNB' and VALID_FLG ='1'and eff_dte between (current date-  10  DAYS) and current date order by eff_dte desc fetch first rows only  with ur";
			String sql3="select  TYPE_ID,UNIT_OFFER_PRICE,UNIT_BID_PRICE,UNIT_VIRTUAL_FUND,(select CODE_Desc from ODSCIF.CD_CODE_T B where A.COMPANY_CODE =B.COMPANY_CODE and A.UNIT_VIRTUAL_FUND =B.DESC_ITEM and B.code_scheme ='T5515' ),EFF_DTE, UPDTE_DTE from ODSCIF.CD_IDX_VALUE_T A where company_code='1' and UNIT_VIRTUAL_FUND = 'FUNC' and VALID_FLG ='1'and eff_dte between (current date-  10  DAYS) and current date order by eff_dte desc fetch first rows only  with ur";
			String sql4="select  TYPE_ID,UNIT_OFFER_PRICE,UNIT_BID_PRICE,UNIT_VIRTUAL_FUND,(select CODE_Desc from ODSCIF.CD_CODE_T B where A.COMPANY_CODE =B.COMPANY_CODE and A.UNIT_VIRTUAL_FUND =B.DESC_ITEM and B.code_scheme ='T5515' ),EFF_DTE, UPDTE_DTE from ODSCIF.CD_IDX_VALUE_T A where company_code='1' and UNIT_VIRTUAL_FUND = 'FUND' and VALID_FLG ='1'and eff_dte between (current date-  10  DAYS) and current date order by eff_dte desc fetch first rows only  with ur";
			stmt=con.createStatement();
			...
			rs=stmt.executeQuery(sql2);
			while(rs.next()){//指数增强型账户
				mairujia2= rs.getObject(2).toString();
				maichujia2= rs.getObject(3).toString();   
				
			}
			rs=stmt.executeQuery(sql3);
			while(rs.next()){//平衡配置型账户
				mairujia3= rs.getObject(2).toString();
				maichujia3= rs.getObject(3).toString();  
			}
			rs=stmt.executeQuery(sql4);
			...
			String outString="Y|"+"中英人寿《金彩连连投资连结保险B款》、《金芒果投资连结保险B款》" +
					"价格公告如下：计价日"+","+date+","+
					"成长型账户买入价"+","+mairujia+",卖出价"+","+maichujia+","+
					"指数增强型账户买入价"+","+mairujia2+",卖出价"+","+maichujia2+","+
					"平衡配置型账户买入价"+","+mairujia3+",卖出价"+","+maichujia3+","+
					"避险型投资帐户买入价"+","+mairujia4+",卖出价"+","+maichujia4+","+
					"本次投资单位价格公告，仅反应各投资帐户以往的投资业绩，并不代表未来的投资收益。";
			logger.info(outString);
			out.append(outString+"重听   请按零,  返回上层菜单请按星号键, 结束请挂机");
		} catch (ClassNotFoundException e) {
	 ...
	 * commandNo--333333
	 * 查询月红利存储利率,合成语音
	 * <p>1.0.0.0 罗尧创建 2010-12-24</p>
	 * @see com.crm.avivacofco.In.CacheBean.AbstractCacheBean#ExistJing()
	 */
	@Override
	public String ExistJing() {
		//System.out.println("月红利存储利率查询方法------333333");
		logger.info("-----月红利存储利率查询------333333");
		System.out.println("操作时间！--"+new Date());
		StringBuffer out=new StringBuffer();
		CacheMgr cacheMgr = CacheMgr.getInstance();
		String channelNo = super.getChannelNo();
		...
		try {
			...
			String sql="select title from CMS_ARCHIVE where catid = 4881 order by id desc fetch first 1 rows only with ur";
			String sql2="select subtitle from CMS_ARCHIVE where catid = 4881 order by id desc fetch first 1 rows only with ur";
			...
			rs=stmt.executeQuery(sql2);
			while(rs.next()){
				lilv=(String) rs.getObject(1);   //利率
			System.out.println(lilv);
			}
			
			
			String outString="Y|"+date+"红利储存利率"+lilv;
			logger.info(outString);
			out.append(outString+"，重听   请按零,  返回上层菜单请按星号键, 结束请挂机");
		} catch (ClassNotFoundException e) {
			// TODO Auto-generated catch block
	 ...
	 * 查询借款利率,合成语音
	 * <p>1.0.0.0 罗尧创建 2010-12-24</p>
	 * @see com.crm.avivacofco.In.CacheBean.AbstractCacheBean#BorrowFunds()
	 */
	@Override
	public String BorrowFunds() {
		//System.out.println("红利储存利率查询方法------444444");
		logger.info("-----红利储存利率查询------444444");
		System.out.println("操作时间！--"+new Date());
		StringBuffer out=new StringBuffer();
		CacheMgr cacheMgr = CacheMgr.getInstance();
		String channelNo = super.getChannelNo();
		...
		try {
			...
			rs=stmt.executeQuery(sql2);
			while(rs.next()){
				 y=(String) rs.getObject(1);   //万能险
			System.out.println(y);
			}
			rs=stmt.executeQuery(sql3);
			while(rs.next()){
				 z=(String) rs.getObject(1);   //传统寿险
			System.out.println(z);
			}
			
			String outString="Y|"+date+"万能险，月借款利率"+y+","+date+"传统寿险，月借款利率"+z+"，重听   请按零,  返回上层菜单请按星号键, 结束请挂机";
			logger.info(outString);
	 ...
	 * 查询留存满一年生存金适用利率,合成语音
	 * <p>1.0.0.0 罗尧创建 2010-12-26</p>
	 * @see com.crm.avivacofco.In.CacheBean.AbstractCacheBean#SubsistOverOneYear()
	 */
	@Override
	public String SubsistOverOneYear() {
		//System.out.println("红利储存利率查询方法------555555");
		logger.info("-----留存满一年生存金适用利率查询------555555");
		System.out.println("操作时间！--"+new Date());
		StringBuffer out=new StringBuffer();
		CacheMgr cacheMgr = CacheMgr.getInstance();
		String channelNo = super.getChannelNo();
		...
		try {
			String date=null;
			String overyear=null;
			String unyear=null;
			Class.forName("com.ibm.db2.jcc.DB2Driver");
			con = DriverManager.getConnection("jdbc:db2://10.145.6.130:50000/PLATSYS","platformsys","platformsys");
			String sql="select title from CMS_ARCHIVE where catid = 4883 order by id desc fetch first 1 rows only with ur";
			String sql2="select author from CMS_ARCHIVE where catid = 4883 order by id desc fetch first 1 rows only with ur";
			String sql3="select source from CMS_ARCHIVE where catid = 4883 order by id desc fetch first 1 rows only with ur";
			stmt=con.createStatement();
			rs=stmt.executeQuery(sql);
			while(rs.next()){
				date=(String) rs.getObject(1);
			...
			rs=stmt.executeQuery(sql2);
			while(rs.next()){
				overyear=(String) rs.getObject(1);   //留存金满一年利率
			System.out.println(overyear);
			}
			rs=stmt.executeQuery(sql3);
			while(rs.next()){
				 unyear=(String) rs.getObject(1);   //留存金满一年利率
			System.out.println(unyear);
			}
			
String outString="Y|"+date+","+"留存满一年"+","+"生存金适用利率"+overyear+"，"+date+","+"留存不满一年"+","+"生存金适用利率"+unyear;
logger.info(outString);			
out.append(outString+"，重听   请按零,  返回上层菜单请按星号键, 结束请挂机");
		} catch (ClassNotFoundException e) {
			// TODO Auto-generated catch block
